...
def load_config() -> dict:
    default = {
        "active_ai":        "groq",
        "trigger":          "+",
        "owner_trigger":    ".",    # триггер только для владельца (ты)
        "public_trigger":   "+",   # триггер для остальных
        "whitelist_on":     False,  # если True — только люди из whitelist могут использовать +
        "whitelist":        [],     # список user_id кому разрешён +
        "blacklist":        [],     # список user_id кому запрещён +
        "memory_on":        True,
        "memory_depth":     8,
        "history_depth":    20,
        "autoreply_on":     False,
...
def add_episode(chat_id: int, event_type: str, content: str, meta: dict = None):
    ...
    # Храним последние 50 эпизодов на чат
