...
async def load_config_async() -> dict:
    """Читает конфиг из PostgreSQL (приоритет) или JSON файла"""
    cfg = dict(DEFAULT_CONFIG)
    # Сначала JSON как fallback
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                cfg.update(json.load(f))
        except: pass
    # Потом DB — перезаписывает (самые актуальные данные)
    try:
...
def load_config() -> dict:
    """Синхронная версия — возвращает кэш (обновляется async)"""
    if _config_cache:
        return dict(_config_cache)
    # Fallback только на JSON если кэш пуст
    cfg = dict(DEFAULT_CONFIG)
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
...
def save_config(cfg: dict):
    """Сохраняет в JSON И в PostgreSQL"""
    _config_cache.update(cfg)
    # JSON
    try:
...
async def _save_config_db(cfg: dict):
    try:
        ...
        # Также сохраняем timestamp чтобы userbot знал что конфиг обновился
        await kv_set("config_updated_at", {"ts": __import__("time").time()})
        log.info(f"✅ Конфиг сохранён в DB: autoreply={cfg.get('autoreply_on')} pm={cfg.get('pm_autoreply')}")
...
async def config_refresh_loop():
    """Каждые 30 сек обновляет кэш конфига из DB"""
    while True:
