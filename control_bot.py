...
async def set_ai(cb: CallbackQuery):
    ...
    cfg = load_config(); cfg["active_ai"] = model; save_config(cfg)
...
async def save_ar_text(msg: Message, state: FSMContext):
    cfg=load_config(); cfg["autoreply_text"]=msg.text; save_config(cfg)
    await state.clear()
...
async def save_wl(msg: Message, state: FSMContext):
    try:
        uid=int(msg.text.strip()); cfg=load_config()
...
async def menu_persona(cb: CallbackQuery):
    cfg=load_config(); on=cfg.get("persona_on",False)
...
async def persona_preset(cb: CallbackQuery):
    ...
    emoji,name,desc=PERSONA_PRESETS[p]; cfg=load_config()
...
async def save_persona_desc(msg: Message, state: FSMContext):
    data=await state.get_data(); cfg=load_config()
...
async def num_mem(cb: CallbackQuery, state: FSMContext):
    ...
    label,key,mn,mx=labels[cb.data]; cfg=load_config(); cur=cfg.get(key,8)
...
async def menu_security(cb: CallbackQuery):
    cfg=load_config(); mat=cfg.get("mat_filter",True); fa2=cfg.get("2fa_on",False)
...
async def menu_stats(cb: CallbackQuery):
    cfg=load_config(); s=cfg.get("stats",{})
...
async def num_settings(cb: CallbackQuery, state: FSMContext):
    ...
    label,key,mn,mx=labels[cb.data]; cfg=load_config(); cur=cfg.get(key,0)
...
async def save_numeric(msg: Message, state: FSMContext):
    ...
    try:
        ...
        cfg=load_config(); cfg[key]=val; save_config(cfg); await state.clear()
...
async def cmd_status(msg: Message):
    cfg=load_config(); ai=cfg.get("active_ai","groq")
