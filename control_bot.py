...
async def save_config_async(cfg: dict):
    """ГЛАВНАЯ функция сохранения — await обязателен для гарантии записи в DB"""
    # 1. Кэш
    _config_cache.clear()
    _config_cache.update(cfg)
    # 2. JSON
    ...
    # 3. DB — AWAIT гарантирует запись до ответа пользователю
    try:
        await kv_set("config", cfg)
        log.info(f"✅ Config saved: autoreply={cfg.get('autoreply_on')} pm={cfg.get('pm_autoreply')} ai={cfg.get('active_ai')}")
    except Exception as e:
        log.error(f"Config DB save error: {e}")

def save_config(cfg: dict):
    """Синхронная обёртка — планирует async сохранение"""
    _config_cache.clear()
    _config_cache.update(cfg)
    try:
        with open(CONFIG_FILE, "w", encoding="utf-8") as f:
...
async def _save_config_db(cfg: dict):
    try:
        await kv_set("config", cfg)
        log.info(f"✅ DB saved: autoreply={cfg.get('autoreply_on')} pm={cfg.get('pm_autoreply')}")
    except Exception as e:
        log.error(f"Config DB save error: {e}")
...
async def set_ai(cb: CallbackQuery):
    ...
    model = cb.data.replace("set_ai_",""); cfg = await load_config_async(); cfg["active_ai"] = model; await save_config_async(cfg)
...
async def save_ar_text(msg: Message, state: FSMContext):
    cfg=await load_config_async(); cfg["autoreply_text"]=msg.text; await save_config_async(cfg)
    await state.clear()
...
async def handle_access(cb: CallbackQuery, state: FSMContext):
    ...
    if act=="acc_open":
        cfg["all_blocked"]=False; cfg["whitelist_on"]=False; await save_config_async(cfg)
        await cb.answer("🔓  Открыто для всех",show_alert=True); await menu_access(cb)
    elif act=="acc_close":
        cfg["all_blocked"]=True; await save_config_async(cfg)
        await cb.answer("🔴  Закрыто для всех",show_alert=True); await menu_access(cb)
    elif act=="acc_wl_on":
        cfg["whitelist_on"]=not cfg.get("whitelist_on",False); await save_config_async(cfg)
...
async def save_wl(msg: Message, state: FSMContext):
    try:
        ...
        cfg["whitelist"]=wl; await save_config_async(cfg); await state.clear()
...
async def save_bl(msg: Message, state: FSMContext):
    try:
        ...
        cfg["blacklist"]=bl; await save_config_async(cfg); await state.clear()
...
async def persona_toggle(cb: CallbackQuery):
    ...
    cfg["persona_on"]=not cfg.get("persona_on",False); await save_config_async(cfg)
...
async def persona_preset(cb: CallbackQuery):
    ...
    await save_config_async(cfg); await cb.answer(f"✅  {emoji} {name}",show_alert=True); await menu_persona(cb)
...
async def save_persona_desc(msg: Message, state: FSMContext):
    ...
    cfg["persona_name"]=data["name"]; cfg["persona_desc"]=msg.text; await save_config_async(cfg)
    await state.clear()
...
async def save_numeric(msg: Message, state: FSMContext):
    ...
    try:
        ...
        cfg=await load_config_async(); cfg[key]=val; await save_config_async(cfg); await state.clear()
