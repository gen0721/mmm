        ...
        try:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=15)) as r:
                    ...
                    if r.status == 200 and resp_text.strip().startswith("{"):
                        import json as _jw
                        data = _jw.loads(resp_text)
            ...
            try:
                ...
                if fn:
                    ans = await fn(
                        [{"role":"user","content":f"Погода сейчас в {city}?"}],
                        "Дай краткий прогноз погоды: температура, описание, влажность. Эмодзи. 3-4 строки."
                    )
                    await message.edit_text(f"🌤 **{city}**\n\n{clean_text(ans)}")
                else:
                    await message.edit_text(f"❌ Не удалось получить погоду для {city}")
            except Exception as e2:
                await message.edit_text(f"❌ Ошибка: {str(e2)[:80]}")
        return
