...
async def config_sync_loop():
    """Каждые 3 секунды читает конфиг из DB — мгновенно подхватывает изменения от control_bot"""
    global config
        ...
        try:
            if USE_DB:
                ...
                if db_cfg and isinstance(db_cfg, dict) and len(db_cfg) > 3:
                    ...
                    if db_cfg.get("autoreply_on") != config.get("autoreply_on") or                        db_cfg.get("pm_autoreply") != config.get("pm_autoreply") or                        db_cfg.get("active_ai") != config.get("active_ai"):
                        log.info(f"🔄 Конфиг обновлён из DB: autoreply={db_cfg.get('autoreply_on')} pm={db_cfg.get('pm_autoreply')} ai={db_cfg.get('active_ai')}")
