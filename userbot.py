...
def load_config() -> dict:
    """Возвращает текущий глобальный конфиг (синхронизируется с DB каждые 5 сек)"""
    global config
    # Если config уже инициализирован — возвращаем его (он синхронизируется из DB)
    try:
        if config:
            return config
    except NameError:
    ...
    # Сначала пробуем JSON (синхронно при старте)
...
def save_config(cfg):
    """Сохраняет конфиг в JSON И в PostgreSQL (общее хранилище для обоих сервисов)"""
    try:
        with open(CONFIG_FILE, "w", encoding="utf-8") as f:
            json.dump(cfg, f, ensure_ascii=False, indent=2)
    except Exception as e:
        log.debug(f"Config JSON save error: {e}")
    # ВАЖНО: пишем в DB — control_bot читает конфиг отсюда
    if USE_DB:
        try:
            ...
            if loop.is_running():
                ...
                loop.run_until_complete(_async_save_config(cfg))
        except Exception as e:
            log.debug(f"Config DB save error: {e}")
...
async def config_sync_loop():
    """Каждые 5 секунд читает конфиг из DB — подхватывает изменения от control_bot"""
    global config
    ...
    while True:
        try:
            if USE_DB:
                ...
                if db_cfg and isinstance(db_cfg, dict) and len(db_cfg) > 1:
                    config.update(db_cfg)
                    log.debug(f"Config synced: autoreply={config.get('autoreply_on')} pm={config.get('pm_autoreply')}")
        except Exception as e:
            log.debug(f"Config sync error: {e}")
        await asyncio.sleep(5)  # каждые 5 секунд

async def load_config_from_db() -> dict:
    """Загружает конфиг из DB (вызывается при старте)"""
    try:
...
async def handle_deleted(client: Client, messages):
    global config
    global config  # использует синхронизированный конфиг из DB
    ...
    async def main():
        ...
        if db_ok:
            ...
            if db_cfg and isinstance(db_cfg, dict):
                config.update(db_cfg)
                log.info(f"✅ Конфиг загружен из PostgreSQL: {list(db_cfg.keys())[:5]}")
        else:
